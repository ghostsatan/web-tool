...
window.Addtabs = {
    ...
    add: function (opts) {
        ...
        if (!$("#" + id).length) {
            ...
            var title = $('<li>', {
                ...
                $('<a>', {
                    ...
                    'role': 'tab',
                    'data-toggle': 'tab'
                }).html((opts.title.indexOf("(") > 0 ? opts.title.substring(0, opts.title.indexOf("(")) : opts.title))
            );
        ...
        if (opts.content) {
            ...
            content.append("<div style='position:fixed;z-index:-1'>页面加载中...</div>").append(
                $('<iframe>', {
                    'class': 'iframeClass iframe_mar_top',
                    'name': 'formname' + opts.id,//  post对象
        ...
        //如果传递post参数：postParams
        if (opts.url && opts.postParams) {
            var oForm = document.createElement("form");
            oForm.type = "hidden";
            oForm.id = 'formname' + opts.id;
            oForm.method = "POST";
            oForm.action = opts.url;
            oForm.target = 'formname' + opts.id;//注意创建的iframe里面的name值
            var oIpt;
            for (var tmpName in opts.postParams) {
                oIpt = document.createElement("input");
                oIpt.type = "hidden";
                oIpt.name = tmpName;
                oIpt.value = opts.postParams[tmpName];
    ...
    pop: function (id, e) {
        ...
        popHtml.css({
            'top': e[0].offsetHeight + 70 + 'px',
            'margin-left': e[0].offsetLeft + 10 + 'px'
        });
